...
pub struct IovecRead {
  pub buf : *mut libc::c_uchar, pub size : usize,
} pub struct IovecWrite {
  pub buf : *const libc::c_uchar, pub size : usize,
...
impl WasiSockaddr {
  pub fn new (family : AddressFamily, sa_data : &mut String)->WasiSockaddr {
    WasiSockaddr {
    family:
      AddressFamily::Inet4, sa_data_len : 14, sa_data : sa_data.as_mut_ptr(),
    }
  }
}
...
pub fn sock_recv(fd
                 : u32, buf
                 : *const IovecRead, buf_len
                 : usize, flags
                 : u16, recv_len
                 : *mut usize, oflags
                 : *mut usize, )
    ->u32;
pub fn sock_recv_from(fd
                      : u32, buf
                      : *mut u8, buf_len
                      : u32, addr
                      : *mut u8, addr_len
...
pub fn sock_send(fd
                 : u32, buf
                 : *const IovecWrite, buf_len
                 : u32, flags
                 : u16, send_len
                 : *mut u32, )
    ->u32;
pub fn sock_send_to(fd
                    : u32, buf
                    : *const u8, buf_len
                    : u32, addr
                    : *const u8, addr_len
...
pub fn get_addrinfo(node
                    : *const u8, node_len
                    : u32, server
                    : *const u8, server_len
                    : u32, hint
                    : *const WasiAddrinfo, res
                    : *mut u32, max_len
                    : u32, res_len
                    : *mut u8, )
    ->u32;
...
impl WasiAddrinfo {
  pub fn default()->WasiAddrinfo {
    WasiAddrinfo {
    ai_flags:
      AiFlags::AI_PASSIVE, ai_family : AddressFamily::Inet4,
  ...
  pub fn new (ai_flags
              : AiFlags, ai_family
              : AddressFamily, ai_socktype
              : SocketType, ai_protocol
              : AiProtocol, ai_addr
              : &mut WasiSockaddr, ai_addrlen
              : usize, ai_canonname
              : &mut String, ai_canonnamelen
              : usize, ai_next
              : *mut WasiAddrinfo)
      ->WasiAddrinfo {
    WasiAddrinfo {
    ai_flags:
      ai_flags, ai_family : ai_family,
  ...
  pub fn get_addrinfo(node
    ...
    let mut wasiaddrinfo_array : Vec<WasiAddrinfo> =
                                     Vec::<WasiAddrinfo>::new ();
    let mut ai_canonname_array : Vec<String> = Vec::<String>::new ();
        for
          i in 0..max_reslen {
            sockbuff.push(String::new ());
            sockaddr_array.push(
                WasiSockaddr::new (AddressFamily::Inet4, &mut sockbuff[i]));
            ai_canonname_array.push(String::new ());
            wasiaddrinfo_array.push(WasiAddrinfo::new (
                AiFlags::AI_PASSIVE, AddressFamily::Inet4, SocketType::Datagram,
                AiProtocol::IPPROTO_TCP, &mut sockaddr_array[i], 0,
                &mut ai_canonname_array[i], 30, std::ptr::null_mut()));
          }
        ...
        unsafe {
          return_code = get_addrinfo(node.as_ptr(), node.len() as u32,
                                     service.as_ptr(), service.len() as u32,
                                     hints, &mut res, 10, &mut res_len);
        };
...
trait AsRawFd { fn as_raw_fd(&self)->u32; }

#[derive(Copy, Clone)]
struct SocketHandle(u32);

impl AsRawFd for SocketHandle {
  fn as_raw_fd(&self)->u32 { self .0 }
}
...
pub struct TcpListener {
  fd : SocketHandle, address : WasiAddress, port : u16,
}
...
impl TcpStream {
  ...
  pub fn connect<A : ToSocketAddrs>(addrs : A)->Result<TcpStream> {
    match addrs.to_socket_addrs()
        ?.find_map(| addrs | unsafe {
            let mut fd : u32 = 0;
            sock_open(AddressFamily::from(addrs) as u8,
                      SocketType::Stream as u8, &mut fd, );
  ...
  pub fn shutdown(&self, how : Shutdown)->Result<()> {
    unsafe { sock_shutdown(self.as_raw_fd(), how as u8); }
    Ok(())
  }
}
...
impl Read for TcpStream {
  fn read(&mut self, buf : &mut[u8])->Result<usize> {
    ...
    unsafe {
      sock_recv(self.as_raw_fd(), &mut vec, 1, flags, &mut recv_len,
                &mut oflags, );
    };
    Ok(recv_len)
  }
...
impl TcpListener {
  ...
  pub fn incoming(&self)->Incoming <'_> { Incoming {
  listener:
    self
  }
}
...
impl UdpSocket {
  ...
  /// If multiple address is given, the first successful socket is
  /// returned.
  pub fn bind<A : ToSocketAddrs>(addrs : A)->Result<UdpSocket> { todo !(); }
  pub fn recv_from(&self, buf : &mut[u8])->Result<(usize, SocketAddr)> {
    let mut addr_len : u32 = 0;
    let mut addr_buf = [0; 32];
    let size = unsafe{sock_recv_from(
        self.as_raw_fd(), buf.as_ptr() as * mut u8, buf.len() as u32,
        addr_buf.as_ptr() as * mut u8, &mut addr_len, 0, )} as usize;
  ...
  pub fn send_to<A : ToSocketAddrs>(&self, buf
                                    : &[u8], addr
                                    : A)
      ->Result<usize> {
    let addr = addr.to_socket_addrs()
        ?.next().ok_or(std::io::Error::new (std::io::ErrorKind::InvalidInput,
                                            "No address.", ));
        let addr_s = CString::new(addr?.to_string()).expect("CString::new");
        let sent =
            unsafe{sock_send_to(self.as_raw_fd(), buf.as_ptr() as *const u8,
                                buf.len() as u32, addr_s.as_ptr() as *const u8,
                                addr_s.as_bytes().len() as u32, 0, )} as usize;
        Ok(sent)
  }
}
